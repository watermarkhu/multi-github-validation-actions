# multi-github-validation-actions
...
Two composite actions wire together: `push` on the upstream side, and
`report` on the target side.

**Upstream** — check out the repo, then call the `push` action on your PR
trigger:
...
jobs:
  cross:
    ...
    steps:
      ...
      - uses: your-org/multi-github-validation-workflows/push@v1
        with:
          ref: ${{ github.event.pull_request.head.sha }}
          ...
          mode: pr
          sign_commit: "true"
          upstream_app_id: ${{ vars.UPSTREAM_APP_ID }}
...
jobs:
  qualify:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: your-org/multi-github-validation-workflows/report@v1
        with:
          state: started
          app_id: ${{ vars.UPSTREAM_APP_ID }}
...
repo, the `push` action skips automatically.
...
| Target app | Target GitHub (B) | Target repo | Upstream repo (A) | `push` action | `contents: write`, `pull_requests: write` |
...
The `push` action expects the upstream commit to already be checked out
in the workspace (use `actions/checkout` first).

| Input | Type | Default | Description |
|-------|------|---------|-------------|
| `ref` | string | *required* | Git ref or SHA to push to the target. Must be reachable from the checked-out workspace. |
...
| `mode` | string | `pr` | `direct` (push to a branch) or `pr` (push and open a PR). |
...
| `workspace` | string | `$GITHUB_WORKSPACE` | Repo working directory. |
| `upstream_app_id` | string | *required* | GitHub App ID registered on the upstream GitHub instance. |
| `upstream_private_key` | string | *required* | PEM-encoded private key for the upstream app. |
| `target_app_id` | string | *required* | GitHub App ID registered on the target GitHub instance. |
| `target_private_key` | string | *required* | PEM-encoded private key for the target app. |

Outputs: `skipped`, `check_run_id`, `target_branch`, `target_url`.
...
1. The `push` action skips when `target_server_url + repository` matches
   the current `github.server_url + github.repository`. This makes the
   same caller workflow safe to install on every instance.
